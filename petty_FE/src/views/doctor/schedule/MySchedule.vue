  ...
  <div class="p-6 space-y-6">
    ...
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button
          @click="changeMonth(-1)"
          class="h-9 px-3 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm font-medium"
        >
          ← Trước
        </button>
        <span class="text-sm font-semibold text-black min-w-[110px] text-center">
      ...
      <div class="grid grid-cols-7">
        ...
        >
          ...
          <div v-if="cell.shift" class="mt-0.5">
            <span
              class="block text-[10px] font-semibold px-1.5 py-0.5 rounded text-white"
          ...
          <div v-if="cell.leave" class="mt-0.5">
            <span
              class="block text-[10px] font-semibold px-1.5 py-0.5 rounded"
              :class="{
                'bg-red-100 text-red-700': cell.leave === 'da_duyet',
                'bg-yellow-100 text-yellow-700': cell.leave === 'cho_duyet',
                'bg-gray-100 text-gray-500': cell.leave === 'tu_choi',
              }"
            >
    ...
    <div class="bg-white border border-gray-200 rounded-xl p-5 space-y-4">
      ...
      <div class="flex gap-3 flex-wrap">
        <div class="flex flex-col gap-1">
          ...
          <input
            v-model="leaveForm.ngay_nghi"
            type="date"
            class="h-9 border border-gray-300 rounded-lg px-3 text-sm focus:outline-none focus:border-[#009689]"
          />
        </div>
        ...
        <div class="flex items-end">
          <button
            @click="submitLeave"
            :disabled="submitting"
            class="h-9 px-4 bg-[#009689] hover:bg-[#007a6e] text-white text-sm font-semibold rounded-lg disabled:opacity-50 transition-colors"
          >
      ...
      <div v-else class="space-y-2">
        ...
        >
          <div>
            ...
            <p v-if="item.ly_do_tu_choi" class="text-xs text-red-500 mt-0.5">
          ...
          <div class="flex items-center gap-2">
            <span
              ...
              :class="{
                'bg-yellow-100 text-yellow-700': item.trang_thai === 'cho_duyet',
                'bg-green-100 text-green-700': item.trang_thai === 'da_duyet',
                'bg-red-100 text-red-700': item.trang_thai === 'tu_choi',
              }"
            >
              {{ item.trang_thai === 'cho_duyet' ? 'Chờ duyệt' : item.trang_thai === 'da_duyet' ? 'Đã duyệt' : 'Từ chối' }}
            </span>
            <button
              v-if="item.trang_thai === 'cho_duyet'"
  ...
  try {
    ...
    if (scheduleRes?.data?.schedule) {
      scheduleRes.data.schedule.forEach((day) => {
        if (day.lich_lam_viec?.length > 0) {
          scheduleMap.value[day.date] = day.lich_lam_viec[0].thoi_gian_truc
        }
      })
    }
    ...
    leaves.value.forEach((l) => {
      leaveMap.value[l.ngay_nghi] = l.trang_thai
    })
  } finally {
    loading.value = false
  }
...
const calendarCells = computed(() => {
  ...
  const firstDow = new Date(y, m - 1, 1).getDay() // 0=Sun,1=Mon…
  ...
  for (let d = 1; d <= days; d++) {
    ...
    cells.push({
      ...
      shift: scheduleMap.value[dateStr] || null,
      leave: leaveMap.value[dateStr] || null,
    })
  }
...
const submitLeave = async () => {
  leaveError.value   = ''
  leaveSuccess.value = ''
  if (!leaveForm.value.ngay_nghi) {
    leaveError.value = 'Vui lòng chọn ngày nghỉ'
    return
  }
