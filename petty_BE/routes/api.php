...
// ── Cập nhật StatisticController để dùng thanh_toans thay vì lich_hens ──
// Sửa route statistics (đã có sẵn)
Route::get('/statistics/dashboard', [\App\Http\Controllers\Api\StatisticController::class, 'getDashboardData'])
    ->middleware(['auth:sanctum', 'staff.only']);
// Thanh toán
Route::post('/thanh-toan/preview', [ThanhToanController::class, 'preview'])->middleware('staff.only');
Route::post('/thanh-toan', [ThanhToanController::class, 'store'])->middleware('staff.only');
Route::get('/thanh-toan/{id}', [ThanhToanController::class, 'show'])->middleware('staff.only');
